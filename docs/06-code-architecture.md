...
- **发布**：tag → 各平台 build job 产出唯一命名的平台 artifact 与 updater manifest fragment → publish job 校验后聚合 SHA256 和唯一 `latest.json`。Windows 使用 Tauri 2 原生 updater 格式：同一个 NSIS x64 `.exe` 同时作为手动安装和 updater 下载资产，旁边发布对应 `.exe.sig`，不生成仅供 Tauri v1 迁移的 legacy `.nsis.zip`；Windows 与 macOS 复用仓库现有 updater 密钥。macOS 直发使用 Apple `Developer ID Application`，不是 Mac App Store 的 `Apple Distribution`；build job 先导入临时 keychain 中的 `.p12`，在本地 runtime dylib 注入后由内到外签名所有 Mach-O，签名启用 Hardened Runtime、安全时间戳并使用 `src-tauri/Entitlements.plist`，随后对最终 `.app` 做 notarization 并 stapling，再生成 DMG 与 updater archive。构建前先从原生 release 产物、Microsoft VC tools redist 和固定版本/哈希的 Vulkan Loader 包生成 runtime staging，Tauri Windows resource map 将其安装到 EXE 同目录。Tauri `.sig` 是更新完整性签名，Apple code signature 是 macOS 发布者身份，二者独立。SignPath/Authenticode 可后置，但构建链必须保留稳定签名插入点。更新通道分 stable/nightly，任何平台 fragment 不得同名覆盖或跨通道引用。
