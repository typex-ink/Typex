...
| 本地模型清单/导入 | 内置清单 + 用户清单合并；导入 LLM GGUF / llama ASR GGUF+mmproj / sherpa ONNX+tokens；导入模型删除同步清理用户清单；零配置兜底只选内置已下载模型；内置清单包含高配手动模型（Whisper large-v3、Qwen3.6 27B/35B-A3B、Qwen3.8 27B、Qwen3 14B/30B-A3B/32B）且不进入自动硬件分档；Qwen3.6/Qwen3.8 条目的文件名、精确字节数、SHA-256 与双下载源固定 |
