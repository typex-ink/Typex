...
pub unsafe fn convert(
    ...
    if let Some(event_type) = option_type {
        // Typex patch: 不在监听线程调 create_string_for_key——它内部走
        // TSMGetInputSourceProperty（HIToolbox 输入法 API），macOS 26 起强制
        // 主线程调用（dispatch_assert_queue），在 event tap 线程调用直接 SIGTRAP。
        // Typex 只消费键码（Key 枚举），不需要字符名。
        return Some(Event {
            event_type,
            time: SystemTime::now(),
            name: None,
